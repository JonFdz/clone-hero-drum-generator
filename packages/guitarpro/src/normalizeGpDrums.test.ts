...
<GPIF>
  <Score><Title>Normalization Demo</Title></Score>
...
describe("normalizeGpDrumsXml", () => {
  it("normalizes supported GPIF drum articulations into DrumHit values", () => {
    const result = normalizeGpDrumsXml(syntheticGpif, { filePath: "song.gp", trackIndex: 1 });
    ...
    expect(result.hits[0]).toMatchObject({ tick: 0, piece: "kick", velocity: 95, durationTicks: 0 });
    expect(result.hits[0].source).toMatchObject({ kind: "gpif", trackIndex: 1, rawArticulation: "Kick" });
  });

  it("maps open and closed hi-hats distinctly", () => {
    const result = normalizeGpDrumsXml(syntheticGpif, { trackIndex: 1 });

    expect(result.hits).toContainEqual(expect.objectContaining({ tick: 960, piece: "hihat_closed" }));
    expect(result.hits).toContainEqual(expect.objectContaining({ tick: 1920, piece: "hihat_open" }));
  });
  ...
  it("uses stable default velocity and documented dynamic mapping", () => {
    ...
    expect(result.hits.find((hit) => hit.piece === "hihat_closed")?.velocity).toBe(88);
    expect(result.hits.find((hit) => hit.piece === "hihat_open")?.velocity).toBe(115);
    expect(result.hits.find((hit) => hit.piece === "snare")?.velocity).toBe(80);
  });

  it("throws clearly for invalid track index", () => {
    expect(() => normalizeGpDrumsXml(syntheticGpif, { trackIndex: 99 })).toThrow(/invalid gpif track index 99/i);
  });
  ...
  it("warns but attempts normalization for selected non-drum tracks", () => {
    ...
    expect(result.warnings.join("\n")).toMatch(/not a detected drum candidate/i);
    expect(result.hits).toContainEqual(expect.objectContaining({ piece: "kick" }));
  });
  ...
  it("reports unsupported repeat or alternate-ending timing structures", () => {
    ...
    expect(result.hits).toContainEqual(expect.objectContaining({ piece: "kick" }));
    expect(result.unhandled.join("\n")).toMatch(/unsupported gpif timing structure/i);
    expect(result.unhandled.join("\n")).toMatch(/repeat/i);
    expect(result.unhandled.join("\n")).toMatch(/alternate/i);
  });
