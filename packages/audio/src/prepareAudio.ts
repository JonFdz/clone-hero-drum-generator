...
const invalidOutputFileNameMessage =
	'Packaged audio filename must be a plain .ogg filename, for example "song.ogg".';
...
export async function prepareAudio(
	...
	assertValidOutputFileName(outputFileName);
	...
	try {
		...
		if (isMissingLibvorbisError(error)) {
			await runner(
				ffmpegPath,
				buildFfmpegArgs(input.sourcePath, outputPath, "vorbis"),
			);
		} else {
			throw error;
		}
...
function assertValidOutputFileName(outputFileName: string): void {
	if (
		...
		outputFileName.includes("\\") ||
		outputFileName.includes(":") ||
		extname(outputFileName).toLowerCase() !== ".ogg"
	) {
		throw new Error(invalidOutputFileNameMessage);
	}
}
