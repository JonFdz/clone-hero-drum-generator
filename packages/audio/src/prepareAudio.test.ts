...
describe("prepareAudio", () => {
	...
	it("accepts a plain custom .ogg output filename", async () => {
		const tempDir = await mkdtemp(join(tmpdir(), "chdg-audio-valid-name-"));
		const sourcePath = join(tempDir, "source.ogg");
		const outputDir = join(tempDir, "out");
		await writeFile(sourcePath, "synthetic ogg fixture");
	...
	it.each([
		...
		"nested\\song.ogg",
		"C:\\temp\\song.ogg",
	])("rejects unsafe packaged audio filename %s", async (outputFileName) => {
		const tempDir = await mkdtemp(join(tmpdir(), "chdg-audio-invalid-name-"));
		const sourcePath = join(tempDir, "source.ogg");
		await writeFile(sourcePath, "synthetic ogg fixture");

		await expect(
			prepareAudio({ sourcePath, outputDir: tempDir, outputFileName }),
		).rejects.toThrow(/plain \.ogg filename/i);
	});
	...
	it("falls back to ffmpeg native vorbis when libvorbis is unavailable", async () => {
		...
		const runner: AudioProcessRunner = async (command, args) => {
			...
			if (args.includes("libvorbis")) {
				throw new Error(
					"FFmpeg conversion failed: Unknown encoder 'libvorbis'",
				);
			}
		};
	...
	it("throws a helpful error when the source is missing", async () => {
		...
		await expect(
			prepareAudio({
				sourcePath: join(tempDir, "missing.mp3"),
				outputDir: tempDir,
			}),
